...
interface EnviarWhatsAppButtonProps {
  compra: Compra
  rifa?: Pick<Rifa, 'nombre'> | null
}

export function EnviarWhatsAppButton({ compra, rifa }: EnviarWhatsAppButtonProps) {
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || (typeof window !== 'undefined' ? window.location.origin : 'http://localhost:3000')
  
  // Formatear número de teléfono con +58
  ...
  // Crear mensaje con los datos de la compra
...
${appUrl}?cedula=${compra.comprador_cedula}
...
¡Mucha suerte en el sorteo! 🍀`
  ...
  // Crear URL de WhatsApp (wa.me)
  // Formato: https://wa.me/584141234567?text=mensaje
  // Remover el + del número para el formato de wa.me
  const numeroSinPlus = telefono.replace(/\+/g, '')
  const whatsappUrl = `https://wa.me/${numeroSinPlus}?text=${mensajeCodificado}`
